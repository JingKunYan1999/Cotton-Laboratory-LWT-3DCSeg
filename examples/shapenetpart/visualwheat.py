...
def main(gpu, cfg):
   ...
   # model = pointnet2().cuda()
    #model = pointMLP(3,numpoints).cuda()  #partnums, points
    ...
    ############1#############
    pretrained_path1 = r"D:\pythonproject\Pointnext\examples\shapenetpart\log\shapenetpart\tomato\curvenettomato\checkpoint\ckpt_best.pth"
...
def validate(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    for idx, data in pbar:
        ...
        for v in range(num_votes+1):
            ...
            logits += model(data['pos'], label_one_hot)
        logits /= (num_votes + 1)
        ...
        for item in range(bsize):  # 对batch中每个点云保存
            ...
            individualpath = predictedpath + str(index) + ".txt"
            singlepoints = seg_points[item, :, :]  # 单个点云数据
            savepredictedtxt(singlepoints,individualpath) #保存可视化结果txt
            index = index + 1
