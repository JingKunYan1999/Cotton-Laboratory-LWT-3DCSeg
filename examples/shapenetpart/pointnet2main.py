...
import argparse
import copy
from openpoints.models.pointstack.PointStack import MyPointStack
import yaml
...
def get_part_mious(pred, target, cls, cls2parts,
                  multihead=False,
                  ):
    """Get the Shape IoU
...
def main(gpu, cfg):
   ...
   # if cfg.model.get('decoder_args', None):
    ...
    #model = MyPointStack().to(device)  #partnums, points
   ...
   # print(torch.load(myselfsupervisepath))
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        cfg.epoch = epoch
        train_loss = \
            train_one_epoch(model, train_loader, criterion,segcriterion,
                            optimizer, scheduler, epoch, cfg)
        is_best = False
        if epoch % cfg.val_freq == 0:
            val_ins_miou, val_cls_miou, val_cls_mious = validate_fn(model, val_loader, cfg, criterion)
    ...
    if cfg.get('num_votes', 0) > 0:
        ...
        test_ins_miou, test_cls_miou, test_part_iou, test_cls_mious  = testmetric(model, test_loader, cfg, num_votes=cfg.get('num_votes', 0),
                                 data_transform=voting_transform)
        # with np.printoptions(precision=2, suppress=True):
        #     logging.info(f'Best Epoch {best_epoch},'
    ...
    print("val_loss_arr: ",val_loss)
    print("miou_arr: ", miou_arr)
...
@torch.no_grad()
def validate(model, val_loader, cfg, criterion, num_votes=0, data_transform=None):
    loss_meter = AverageMeter()
    ...
    miou_arr.append(ins_miou.cpu().numpy().tolist())
...
def testmetric(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    ####################3################3
    ins_macc_list, ins_mpre_list, ins_mrecall_list = [], [], []  #####################3
    part_acc, part_pre, part_recall = [], [], []
    ###################3
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser('ShapeNetPart Part segmentation training')  #mydatacfgs.yaml  #spotrdefault
    parser.add_argument('--cfg', type=str, default="D:/pythonproject/Pointnext/cfgs/shapenetpart/pointnet2default.yaml", help='config file')
