...
def main(gpu, cfg):
    ...
    root = 'D:/pythonproject/Pointnext/data/ShapeNetPart/pipelinework'
    numpoints = 10000
    batch_size = 8
...
def validate(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    for idx, data in pbar:
        ...
        leafarr = []
        disarr = []
        predictedpath = "D:/pythonproject/Pointnext/data/ShapeNetPart/pipelinework/predictedply/"
...
def read_ply_cloud(filename):
    ...
    xyzout_arr = np.asarray(pcd.points)
    nxyzout_arr = np.asarray(pcd.normals)
    newxyz = np.concatenate((xyzout_arr, nxyzout_arr), axis=1)

    txtpath = "D:/pythonproject/Pointnext/data/ShapeNetPart/pipelinework/20230515/"
    txtfilename = str(int(calendar.timegm(time.gmtime())) - 1) + ".txt"
    newfilepath = txtpath + txtfilename
    np.savetxt(newfilepath,newxyz)

    filebegins = "shape_data/20230515/"
    usedpath = filebegins + txtfilename[:-4]
    testarr = [usedpath]
    textsetpath = "D:/pythonproject/Pointnext/data/ShapeNetPart/pipelinework/train_test_split/shuffled_test_file_list.json"
    with open(textsetpath,'w') as f:
        f.write(str(testarr).replace('\'', '\"'))


def plytoxyz(oriplypath, norplypath):
    exepath = "D:/pythonproject/pipelineprocessply/point_normal.exe"
    strmsg = exepath + " " + oriplypath + " " + norplypath
    os.system(strmsg)  # 此处已经设置好了该exe文件的环境变量
    read_ply_cloud(norplypath)



if __name__ == "__main__":

    oriplypath = "D:/pythonproject/pipelineprocessply/1.ply"
    norplypath = "D:/pythonproject/pipelineprocessply/new1.ply"
    plytoxyz(oriplypath,norplypath)
