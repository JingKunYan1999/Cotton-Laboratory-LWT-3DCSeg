...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        fps_idx = self.sample_fn(xyz, self.groups).long()
...
class PointStackSeg(nn.Module):
    def __init__(self, activation="relu", bias=False, use_xyz=False, normalize="anchor", **kwargs):
        ...
        assert len(PRE_BLOCKS) == len(K_NEIGHBORS) == len(
            REDUCERS) == len(POS_BLOCKS) == len(DIM_EXP), \
        ...
        for i in range(len(PRE_BLOCKS)):
            out_channel = last_channel * DIM_EXP[i]
            ...
            pre_block_module = PreExtraction(last_channel, out_channel, pre_block_num,
                                             groups=GROUPS,
                                             res_expansion=RES_EXP,
                                             bias=bias, activation=activation, use_xyz=use_xyz)
            self.pre_blocks_list.append(pre_block_module)
            # append pos_block_list
            pos_block_module = PosExtraction(out_channel, pos_block_num, groups=GROUPS,
                                             res_expansion=RES_EXP, bias=bias,
        ...
        self.global_mha = nn.MultiheadAttention(embed_dim=1024 * lq_multiplier, num_heads=16, batch_first=True,
                                                bias=False)
...
class LinearSegmentator(nn.Module):
    def __init__(self):
        ...
        IN_CHANNELS = [4288]
        DIMS = [512, 256]
        self.in_channels = IN_CHANNELS + DIMS[0:-1]
        ...
        self.segmentator.extend(nn.Sequential(
            nn.Conv1d(DIMS[-1], NUM_CLASS, kernel_size=1)
        ))

        self.segmentator = nn.ModuleList(self.segmentator)

        self.cls_map = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=64, kernel_size=1, bias=True),
            nn.BatchNorm1d(64),
            nn.ReLU(),
    ...
    def forward(self, data_dic):
        ...
        # label_one_hot = np.zeros((cls_tokens.shape[0], 1))
        # for idx in range(cls_tokens.shape[0]):
        #     label_one_hot[idx, cls_tokens[idx]] = 1
        # label_one_hot = torch.from_numpy(label_one_hot.astype(np.float32))
        # label_one_hot = label_one_hot.cuda()
        # cls_tokens = label_one_hot

        cls_tokens = to_categorical(cls_tokens, 1)  # (B, 1, 16)
...
class MyPointStack(nn.Module):
    ...
    def build_head(self, model_info_dict):
        ...
        segmentator_module = LinearSegmentator()
    ...
    def forward(self, data_dic):
        ...
        return data_dic['pred_score_logits']