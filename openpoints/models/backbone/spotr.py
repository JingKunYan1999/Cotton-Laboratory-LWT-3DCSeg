        ...
        if self.is_head:
            ...
            if self.use_res or 'df' in self.feature_type:
                ...
                if self.use_global:
                    ...
                    global_dp = rearrange(global_p[:, None, :, :] - new_p[:, :, None, :],
                                          'b n m d -> b d n m').contiguous()  # b 3 m n
                    global_fj = repeat(global_f, 'b d m -> b d n m', n=new_p.size(1)).contiguous()
                    global_fj = get_aggregation_features(new_p, global_dp, fi, global_fj,
                                                         feature_type=self.feature_type)  # (b,d+3,n',m)

                    updated_global_f = new_attention(self.gconvs(global_fj), attn=self.attn_global(global_fj),
                                                     tau=self.tau)  # (b,d+3,n',m) -> (b,d',n')
...
class SPoTrEncoder(nn.Module):
    ...
    def _make_enc(self, block, channels, blocks, stride, group_args, is_head=False,
        ...
        layers.append(SPALPA(self.in_channels, channels,
                             ...
                             group_args=group_args, norm_args=self.norm_args, act_args=self.act_args,
