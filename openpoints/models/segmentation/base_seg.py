...
class BasePartSeg(BaseSeg):
    def __init__(self, encoder_args=None, decoder_args=None, cls_args=None, is_selfsupervise=False, is_localloss=False, **kwargs):
        super().__init__(encoder_args, decoder_args, cls_args, **kwargs)
        self.selfsupervise = is_selfsupervise  #是佛需要自监督
        self.is_localloss = is_localloss
    ...
    def forward(self, p0, f0=None, cls0=None):
        ...
        ### self-supervise
        if self.selfsupervise and self.is_localloss:
            p0 = p[0]
            sa3 = f[5]
            sa1 = f[2]
        ...
        if self.head is not None and not self.selfsupervise:
            f = self.head(f)


        if self.is_localloss:
            return f, sa1, sa3, p0
        else:
            return f
