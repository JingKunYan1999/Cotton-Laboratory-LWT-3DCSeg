...
class ShapeNetPartNormal(Dataset):
    ...
    def __init__(self,
        ...
        self.wi = wi
        self.myvalpoints = 2048   #自定义val一直为2048点测试
        ...
        if presample and not os.path.exists(filename):
            ...
            self.data, self.cls = [], []
            npoints = []
            num_points = self.myvalpoints  #testing set used 2048
            for cat, filepath in tqdm(self.datapath, desc=f'Sample ShapeNetPart {split} split'):
                ...
                ##########newsample code #################### 训练集下采样，测试集不进行下采样
                if self.is_newsample:


                    npoints.append(len(data))
                    if len(data) != 2048:
                        choice = np.random.choice(len(data), num_points, replace=True)
                        data = data[choice]
    ...
    def __getitem__(self, index):
        import random
        self.wi = random.uniform(0.25,0.45)
        ...
        if 'train' in self.split:
            ...
            if self.is_newsample:
                ...
                spikenums = int(self.npoints * self.wi if len(data[data[:,-1]==1])  >= self.npoints * self.wi else len(data[data[:,-1]==1]))
                ...
                if spikenums > 0:
                   ...
                   # print("spikenum",spikenums)
                    ...
                    leafidx = grouper(torch.from_numpy(samplespikedata[:,:3]).to(torch.float32).cuda().unsqueeze(0),
                    ...
                    leafnums = int((self.npoints - spikenums) if len(leafidx) > (self.npoints - spikenums) else len(leafidx))
                    ...
                    if leafnums < len(leafidx):
                       ...
                       # knnleafdata = knnleafdata
                        ...
                        if remaindernums > 0:
                            ...
                            remainderleafdata = torch.from_numpy(remainderleafdata).to(torch.float32).cuda().unsqueeze(
                                0)
                    ...
                    # if remaindernums > 0:
                    #     remainderleafdata = np.delete(leafdata,leafidx,axis=0)
                    #     remainderleafdata = torch.from_numpy(remainderleafdata).to(torch.float32).cuda().unsqueeze(0)
                    #     remainderleafdata = fps(remainderleafdata, remaindernums).cpu().numpy()[0]
                    #     data = np.concatenate((samplespikedata, knnleafdata,remainderleafdata), axis=0)
                    #
                    #
                    ...
                    #logging.info(f"叶穗比:{len(data[data[:, -1] == 1]) / len(data) }")  #(len(data[data[:, -1] == 0]) + len(data[data[:, -1] == 1]))
                    point_set, seg = data[:, :6], data[:, 6].astype(np.int64)
            ...
            ##########orisample code ####################
                data = torch.from_numpy(data).to(
                    torch.float32).cuda().unsqueeze(0)
                data = fps(data, self.npoints).cpu().numpy()[0]
              #  logging.info(f"叶穗比:{len(data[data[:,-1]==1])/(len(data[data[:,-1]==0])+len(data[data[:,-1]==1]))}")
                point_set, seg = data[:, :6], data[:, 6].astype(np.int64)
                choice = np.random.choice(len(seg), self.npoints, replace=True)
                point_set = point_set[choice]
                seg = seg[choice]
            ...
            point_set = point_set[:self.myvalpoints]
            seg = seg[:self.myvalpoints]
